...
class GoogleGenAIClient(ModelClient):
    ...
    def __init__(
        ...
        try:
            ...
            self.sync_client = genai.Client(api_key=api_key_val)
        except ImportError:
    ...
    def call(self, api_kwargs: Dict = {}, model_type: ModelType = ModelType.UNDEFINED):
        ...
        if model_type == ModelType.EMBEDDER:
            ...
            try:
                response = self.sync_client.models.embed_content(
                    model=model,
            ...
            response = self.sync_client.models.generate_content(
                model=model,
                contents=api_kwargs.get("contents")
            )
            return response
    ...
    def parse_embedding_response(self, response: Any) -> EmbedderOutput:
        try:
            # Handle Google's embed_content response (EmbedContentResponse)
            embeddings = []
